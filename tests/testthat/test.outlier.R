test_df = data.frame(
        ...
        Species =c('Iris-setosa', 'Iris-virginica', 'Iris-germanica', 'Iris-setosa', 'Iris-setosa', 'Iris-setosa', 'Iris-setosa', 'Iris-setosa', 'Iris-setosa', 'Iris-setosa', 'Iris-setosa')
)
...
trim_output = data.frame(
        ...
        Species =c('Iris-setosa', 'Iris-virginica', 'Iris-setosa', 'Iris-setosa', 'Iris-setosa', 'Iris-setosa', 'Iris-setosa', 'Iris-setosa')
)
...
test_that("Dataframe arument must be a data.frame", {
    expect_error(outlier_identifier("123", columns = test_column, "mean"))
    expect_error(outlier_identifier(c(1), columns = test_column, "mean"))
})

# columns argument test
test_that("Columns argument is not vector of column names, or contains non-existing or non-numeric columns", {
...
test_that("selected method is not one of c('trim', 'median', 'mean')", {
    ...
    expect_error(outlier_identifier(test_df, columns = test_column, method = 123))
})

# test for value equal
test_that("The outlier function has error values", {
  expect_true(all.equal(outlier_identifier(test_df, columns = test_column), trim_output))
  expect_true(all.equal(outlier_identifier(test_df, columns = test_column, method = "median"), median_output))
  expect_true(all.equal(outlier_identifier(test_df, columns = test_column, method = "mean"), mean_output))
  expect_true(all.equal(outlier_identifier(test_df, columns = c("SepalLengthCm"), method = "mean"), column_output))
})
